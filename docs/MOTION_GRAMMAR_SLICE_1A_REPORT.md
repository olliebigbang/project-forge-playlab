...
Pan and Broom direction were human-confirmed after the shared sprite/anchor/hitbox normalization fix. The Shotgun development sidecar records the generated image's true muzzle and rear endpoint; generic runtime normalization mirrors Sprite, all action anchors and rear contact together. Rule C Hit 3 uses `rear_contact` rather than a Shotgun-name branch.
