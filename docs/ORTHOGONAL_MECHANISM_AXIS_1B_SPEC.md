...
状态：DESIGN ONLY。派生规则经 P13/P15 两轮实测后定形：**`contact_resolution`
只取 `rigidity`，不带质量阈值**。解决 7 对碰撞中的 4 对，其余 3 对无解、已具名记录。
§3 的通道移交不受影响。
...
就这样，没有阈值。P13 与 P15 两轮实测把它削到这个形状：

- **P13**：判据应当是条件分布，不是边缘分布。冲击轴不需要分开本来就挥法不同的物件
  （P12 已证 `contact_surface` 在选择层做了这件事），只需要分开**共享同一连击序列**的
  物件。按此实测，rigidity 分开 3 组碰撞里的 2 组、7 对里的 4 对，且分开的正是动机本身
  （鸡腿出棒球棒+折叠凳，钓鱼竿出木椅+灭火器）。
- **P15**：原设计里那个质量阈值 θ **一对都没解决**。θ=0.538 是在 6 个物件上拟合的，
  放到 18 个物件上 83% 落进同一类；而任何能把类别配平的 θ 都落在 0.011–0.047 的间隔带里，
  全在噪声底 0.065 之下。质量是好的连续输入（P09 就是为此建的），是差的类别切分器。

⚠️ 实现前必须先修 `artifacts/mass_axis_poc/` 的 chicken_leg——`rigidity: rigid` 是手写的
（`author_v1_4_sidecars.py` 注释自承），模型给的是 `semi_rigid`。按手写值鸡腿会留在
rigid 分支跟棒球棒、折叠凳同组，正好是 1B 要破的那个碰撞。

**已知无解的 3 对**（同材质、同序列，契约里没有东西能分开它们）：
长剑/消防斧、棒球棒/折叠凳、木椅/灭火器。这是新信号的立论依据，比 1B 开场时窄得多。
...
| A2 | 改 `real_mass_kg` **不得**改变 `contact_resolution`（P15：质量不再进入本轴） |
