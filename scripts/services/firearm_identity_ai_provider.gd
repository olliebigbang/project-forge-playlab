class_name FirearmIdentityAIProvider
extends RefCounted

const BRIDGE_SCRIPT := "res://tools/semantic/bridge/firearm_identity_ai_bridge.py"
const REQUEST_SCHEMA := "forge-firearm-identity-ai-request-v1"
const RESULT_SCHEMA := "forge-firearm-identity-ai-bridge-result-v1"
const OUTPUT_ROOT := "user://playlab/firearm_identity_ai/requests"
...
func configure(next_python_executable: String = "python") -> Dictionary:
	...
	if not FileAccess.file_exists(bridge_path):
		return _failure("AI_FIREARM_BRIDGE_SCRIPT_MISSING")
...
func request_identity(player_text: String) -> int:
	...
	if DirAccess.make_dir_recursive_absolute(active_output_directory) != OK:
		failure_reason = "AI_FIREARM_REQUEST_DIRECTORY_CREATE_FAILED"
		return active_revision
	var request_path := active_output_directory.path_join("request.json")
	var temporary_path := request_path + ".tmp"
	var file := FileAccess.open(temporary_path, FileAccess.WRITE)
	if file == null:
		failure_reason = "AI_FIREARM_REQUEST_WRITE_FAILED"
		return active_revision
	file.store_string(JSON.stringify({
		"schema": REQUEST_SCHEMA,
		"identity": player_text.strip_edges(),
	}, "  "))
	file.close()
	if DirAccess.rename_absolute(temporary_path, request_path) != OK:
		failure_reason = "AI_FIREARM_REQUEST_ATOMIC_RENAME_FAILED"
		return active_revision
	...
	if process_id <= 0:
		failure_reason = "AI_FIREARM_BRIDGE_PROCESS_START_FAILED"
	return active_revision
...
func poll() -> Dictionary:
	...
	if Time.get_ticks_msec() - started_msec > int(timeout_seconds * 1000.0):
		if process_id > 0 and OS.is_process_running(process_id):
			OS.kill(process_id)
		delivered = true
		return _failure("AI_FIREARM_BRIDGE_TIMEOUT")
	...
	if process_id > 0:
		...
		if Time.get_ticks_msec() - process_exited_msec < 1000:
			return {"status": "running", "revision": active_revision}
		delivered = true
		return _failure("AI_FIREARM_BRIDGE_EXITED_WITHOUT_RESULT")
	return {"status": "running", "revision": active_revision}
...
func _load_result(path: String) -> Dictionary:
	...
	if not parsed is Dictionary:
		delivered = true
		return _failure("AI_FIREARM_BRIDGE_RESULT_INVALID_JSON")
	var result := parsed as Dictionary
	if str(result.get("schema", "")) != RESULT_SCHEMA:
		delivered = true
		return _failure("AI_FIREARM_BRIDGE_RESULT_SCHEMA_INVALID")
	delivered = true
	if str(result.get("status", "")) != "success":
		return _failure(str(result.get("failure_reason", "AI_FIREARM_BRIDGE_FAILED")))
	if not result.get("response", {}) is Dictionary:
		return _failure("AI_FIREARM_BRIDGE_RESPONSE_MISSING")
	return {
		"status": "success",
