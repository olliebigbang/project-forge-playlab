class_name EnemyAIBlueprintProvider
extends RefCounted

const BRIDGE_SCRIPT := "res://tools/semantic/bridge/enemy_ai_blueprint_bridge.py"
const REQUEST_SCHEMA := "forge-enemy-ai-blueprint-request-v1"
const RESULT_SCHEMA := "forge-enemy-ai-blueprint-bridge-result-v1"
const OUTPUT_ROOT := "user://playlab/enemy_ai/requests"
...
func configure(next_python_executable: String = "python") -> Dictionary:
	...
	if not FileAccess.file_exists(bridge_path):
		return _failure("AI_ENEMY_BRIDGE_SCRIPT_MISSING")
	return {"ok": true, "bridge_path": bridge_path, "player_confirmation_required": false}


func request_blueprint(concept: String) -> int:
	cancel_current()
	active_revision += 1
	delivered = false
	failure_reason = ""
	...
	if DirAccess.rename_absolute(temporary_path, request_path) != OK:
		failure_reason = "AI_ENEMY_REQUEST_ATOMIC_RENAME_FAILED"
		return active_revision
	...
	if process_id <= 0:
		failure_reason = "AI_ENEMY_BRIDGE_PROCESS_START_FAILED"
	return active_revision
...
func poll() -> Dictionary:
	...
	if Time.get_ticks_msec() - started_msec > int(timeout_seconds * 1000.0):
		if process_id > 0 and OS.is_process_running(process_id):
			OS.kill(process_id)
		delivered = true
		return _failure("AI_ENEMY_BRIDGE_TIMEOUT")
	...
	if process_id > 0:
		...
		if Time.get_ticks_msec() - process_exited_msec < 1000:
			return {"status": "running", "revision": active_revision}
		delivered = true
		return _failure("AI_ENEMY_BRIDGE_EXITED_WITHOUT_RESULT")
	return {"status": "running", "revision": active_revision}
...
func _load_result(path: String) -> Dictionary:
	...
	if not parsed is Dictionary:
		delivered = true
		return _failure("AI_ENEMY_BRIDGE_RESULT_INVALID_JSON")
	var result := parsed as Dictionary
	if str(result.get("schema", "")) != RESULT_SCHEMA:
		delivered = true
		return _failure("AI_ENEMY_BRIDGE_RESULT_SCHEMA_INVALID")
	delivered = true
	if str(result.get("status", "")) != "success":
		return _failure(str(result.get("failure_reason", "AI_ENEMY_BRIDGE_FAILED")))
	if not result.get("response", {}) is Dictionary:
		return _failure("AI_ENEMY_BRIDGE_RESPONSE_MISSING")
	return {
		"ok": true,
		"status": "success",
