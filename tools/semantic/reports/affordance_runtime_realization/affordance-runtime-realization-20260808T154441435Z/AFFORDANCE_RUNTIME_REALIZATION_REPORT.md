...
- Monotonic mechanism contracts passed: 23/23
- Non-mechanical invariants passed: 2/2
- Frozen profiles executed through the neutral harness: 12/12
- Profile changes that did not reach measured combat behavior: -
- Runtime changes with a wrong or incomplete monotonic direction: -
...
| mass_rear | mass_distribution | PASS | RUNTIME_PROPERTY_PASS | `bash -> sweep -> slam` | - |
...
| feature_stock | has_stock | PASS | RUNTIME_PROPERTY_PASS | `bash -> sweep -> bash` | - |
...
| A12 | `sweep -> spin -> slam` | 91456 | 20 | 200 | 53.63 | 590.26 |

## Diagnosis

All declared anonymous monotonic properties reach the current combat runtime. This is a technical result only; the next human feel validation must use new unseen real assets.
